...
class EncodedCIFAR10(Dataset):
    ...
    def get_encoded_images_and_labels(self):
        ...
        # 自动创建 spikedata 文件夹及子文件夹 CIFAR10_encoded
        encoded_folder_name = "CIFAR10_encoded"
        ...
        for batch_idx in range(num_batches):
            ...
            save_name = f"batch{batch_idx + 1}_encoded_images_{encoding_type_str}_T{self.T}_{dataset_str}_CIFAR10.pt"
            encoded_images_path = os.path.join(encoded_data_folder, save_name)
            save_name_labels = f"batch{batch_idx + 1}_labels_{encoding_type_str}_T{self.T}_{dataset_str}_CIFAR10.pt"
