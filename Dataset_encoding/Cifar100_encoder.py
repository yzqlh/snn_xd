...
class EncodedCIFAR100(Dataset):
    def __init__(self,  encoding_type, T, train=True, transform=None, target_transform=None):
        self.train = train
    ...
    def get_encoded_images_and_labels(self):
        ...
        # 自动创建 spikedata 文件夹及子文件夹 CIFAR100_encoded
        encoded_folder_name = "CIFAR100_encoded"
        ...
        encoded_images_path_list = []
        data = []
        labels = []
        for batch_idx in range(num_batches):

            dataset_str = 'train' if self.train else 'test'
            save_name = f"batch{batch_idx + 1}_encoded_images_{encoding_type_str}_T{self.T}_{dataset_str}_CIFAR100.pt"
            encoded_images_path = os.path.join(encoded_data_folder, save_name)
            save_name_labels = f"batch{batch_idx + 1}_labels_{encoding_type_str}_T{self.T}_{dataset_str}_CIFAR100.pt"
