...
def test_global(device, batch_size, time, dataset_name:str = 'MNIST', encoder:str = 'poisson'):
    with torch.no_grad():
        train_acc = 0
        net =nn.Sequential(
        layer.Conv2d(1,10,5,bias=False),
...
train_global(device=device, batch_size=60, time=30,)
test_global(device=device, batch_size=60, time=30,)
